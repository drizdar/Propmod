...
def engeq(ks,D,rho,mu,z1,z2,L,g,Q,Pump,inf,kLtot):
    ...
    return Tr_use, phd, P, D, V, Pipe_Cost, Pump_Cost, Cost
...
def comboin(MemTyp,TurbTyp,Tr_usei,Q,C_D,C_F,proj,OpTime,Cms,T,Tropp,PT_d,PT_f,C_Vant,PTopp,inf,v,R,A_m,L_m,Mpv,MW_NaCl):
    ...
    n_protr = n_pv/14 #number of process trains (skids)
    n_protr = math.ceil(n_protr) #round up to find total number of process trains
    A_t = n_protr*14*Mpv*A_m #update membrane area to account for rounding
    Q_di = sys_in[3] * n_pv #L/hr - total draw influent
    Q_fi = sys_in[2] * n_pv #L/hr
    Q_mi = Q_di + Q_fi #L/hr total inflow rate
    Q_de = sys_in[1] * n_pv #L/hr - total effluent rate from draw (used for power generation)
    ...
    Tr_use = Tr_usei.calc() * Tropp #calculates energy used in transmission and multiplies values by Tropp
    ...
    SE_net = SE_gross*TurbTyp.W_n - Tr_use[0]*(Q_fi/Q_mi) - SE_PT #kWh/m^3
    if SE_net < 0:
        SE_net = 0 #to make the math work, since SE_net < 0 gives an imaginary number for Turbcost
    #MW_gross2 = (A_t*sys_in[1])/1e6 #MW #only works of Q_d = Q_f
    MW_gross = (SE_gross*Q_de)/1e6
    MW_net = (SE_net*Q_de)/1e6 #MW
    ...
    Cost_Turb = (2600 * ((2 * MW_net * 1000)**0.54) * 1.26) *1.1865 #$ eqn 19b from "Aggidis, G.A., Luchinskaya, E., Rothschild, R., Howard, D.C., 2010. The costs of small-scale hydro power production: Impact on the development of existing potential. Renew Energ 35, 2632–2638."
    #for pt: PT_E, ptcd, ptcf
    Cost_PTd = pt[1]*PT_d.costQ(Q_di,inf) #$
    Cost_PTf = pt[2]*PT_f.costQ(Q_fi,inf) #$
    Cost_PT = Cost_PTd + Cost_PTf #$
    Cost_Cons = (Cost_Memb + Cost_Tr + Cost_Turb + Cost_PT)*1.35 #multiply by 1.35 to add cost of pipes,additional circulation pumps, controls, and site prep, as suggested by McGiveney
    ...
    SE_Ineff  = SE_gross * (1-TurbTyp.W_n)
    SE_Tr = Tr_use[0]*(Q_fi/Q_mi)
    MW_Ineff = (SE_Ineff*Q_de)/1e6
...
def SA_indices(problem,Si,Y,method,csv_title_SI,Notes):
    path = 'SA tests' #folder that test files will be saved to
